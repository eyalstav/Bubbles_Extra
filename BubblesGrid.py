...
def find_bubble_location_in_grid(bullet_bubble):
    #SMOOTH BUT BUGGY
    #print(len(bubbles_grid),len(bubbles_grid[0]))
    #bx = round(bullet_bubble["center_x"])-22
    #by = round(bullet_bubble["center_y"])-22
    #print(bx,by)
    #print(by/43,bx/47)
    #gx = round(bx/47,0)
    #gy = round(by/43,0)
    #print(int(gy),int(gx)-1)
    #return(int(gy),int(gx))
    closest = 999999999
    spot = ()
    #I can just find the closet center from the grid, also make sure its empty and put it there.
    for row in range(len(bubbles_grid)):
        for s in range(len(bubbles_grid[row])):
            if bubbles_grid[row][s]["color"] == "EMPTY":
                d = ((bullet_bubble["center_x"]-float(bubbles_grid[row][s]["center_x"]))**2+(bullet_bubble["center_y"]-float(bubbles_grid[row][s]["center_y"]))**2)**0.5
                if d < closest:
                    closest = d
                    spot = (row,s)
    return spot